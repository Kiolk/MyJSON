...
public class UpdateNoteAsyncTask extends AsyncTask<Pair<Context, String>, Void, String> {
    ...
    protected String doInBackground(Pair<Context, String>... params) {
        ...
        try {
            return myApiService.update(index, newWriter).execute().toString();      //get(index).execute().getName();   //sayHi(name).execute().getData();
        } catch (IOException e) {
            return e.getMessage();
        }
    }
